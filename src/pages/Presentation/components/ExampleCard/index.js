...
function ExampleCard({ image, name, count, pro, ...rest }) {
  const imageTemplate = (
    <MKBox
      ...
      borderRadius="xl"
      shadow="lg"
      minHeight="20rem" // Increased height to make the image larger
      sx={{
        overflow: "hidden",
    ...
    >
      ...
      {/* Apply image as background to cover the card */}
      <MKBox
        component="div"
        sx={{
          ...
          minHeight: "20rem",              // Increased height for larger image
          width: "100%",                   // Ensure the width of the container is 100%
          borderRadius: "xl",              // Keep the rounded corners
          transition: "transform 200ms ease-out", // Animation for hover effect
          "&:hover": {
            transform: "perspective(999px) rotateX(7deg) translate3d(0px, -4px, 5px)",
          },
        }}
